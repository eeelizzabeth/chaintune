...
struct RKMonth: View {
    ...
    // session variable exist only when user logged in
    ...
    func dateTapped(date: Date) {
        if self.isEnabled(date: date) {
            ...
            // display the record of timed duration that selected day from database.
            case 0:
                if self.rkManager.selectedDate != nil &&
                    ...
                    // get records once
                    let docRef = db.collection("users").document("8oe4geRSVIVGMKY31R4sy7Cjxwq1")
                    ...
                    // compare the selected date with db timestamp
